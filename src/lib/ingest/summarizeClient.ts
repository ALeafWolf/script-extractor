...
时序与地点保护规则（极其重要）：
- 保留明确的地点名称（如"枫河露营公园""民宿""办公室"），不得替换为泛指。
- 保留"再次""又""第N次""上回""回到""第二次""前几天""前段时间""几天后"等表示重复访问或时序顺序的线索；不得压缩为一次泛泛的访问。
- 保留事件之间的先后关系（"在……之后""之前""当时""后来"）。
- 如有"回忆""记得""想起""那时"等表述，说明存在记忆/现实切换，须在概要中体现这一切换。
...
时序保护规则：
- 如果多个场景提到同一地点或活动（如"去枫河""住民宿""写信"），必须保留"第几次""再次""又""上回"等重复访问标记，不得合并成一次泛泛的访问。
- 如果场景之间存在明显的先后或因果顺序，须在概要中体现这一推进。
...
时序保护规则：
- 如果同一地点或活动多次出现（如多次前往同一地点），须在摘要中保留"第一次""第二次""再次""又"等区分标记，不得合并成一次泛泛访问。
- 各篇章之间存在时间推进时，须体现"之后""后来""此前"等关系。
